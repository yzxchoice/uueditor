class TabEvent extends eui.Component implements IUUContainer{
	...
	private defaultRelevanceItem: ITrigger = {
		"delay": 100,
		"eventType": 1,
		"sourceId": '8405',
		"sourceType": "e",
	...
	private touchSelection2(evt: egret.TouchEvent){
		...
		if(this.stateObj.selectionVisible){
			...
			let disPlayList: Picture[] = this.editGroup.displayList;
			for(let j = 0, num = disPlayList.length; j < num; j++){
				let item: Picture = disPlayList[j]
				let displayItemData: UUData<any> = item.image.data;
				let relevanceItemId: string = displayItemData.id;
	...
	private onClick_Selection(evt:egret.TouchEvent){
		...
		let selected: boolean = checkoutBox.selected;
		if(selected === undefined) return;
		let relevanceItemId = checkItem.labelText;
		let eventSetMessage: ITrigger = this.relevanceItemIdObj[relevanceItemId];
		if(!eventSetMessage){
	...
	private drawEventSet(eventSetMessage: ITrigger){
	...
	private pushEventSet(eventSetMessage: ITrigger){
		let eventSet:EventSetDome = this.drawEventSet(eventSetMessage);
		eventSet.pushData();
	}
	public removeEventSet(eventSetMessage: ITrigger){
		let relevanceItemId: string = eventSetMessage.targetId;
		let eventSet: EventSetDome = <EventSetDome>this.gp_eventSetContainer.getChildByName(relevanceItemId);
		eventSet.removeData();
	}
	private initShowEventSetList(){
		...
		for(let j = 0, num = disPlayList.length; j < num; j++){
			...
			let isSelected: boolean = this.relevanceItemIdList.indexOf(relevanceItemId) == -1 ? false : true;
			if(isSelected){
				let eventSetMessage: ITrigger = this.relevanceItemIdObj[relevanceItemId];
